...
the ``first-to-be-explored'' atom of the tree and then interpretes it.
We get the this particular node via \pathend,
...
the left subtree, if it exists, or on the right-subtree otherwise.
In the case of a conjunction,
if the \pathend $p$ of the lhs leads to a \okC node then we look for 
...
The \steptag (see~\cref{type:steptag}) indicates the type of internal tree step 
...
that is, it transforms the tree if the its \pathend is an atom,
