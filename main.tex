...
\begin{figure}[H]
  \begin{subfigure}{0.5\textwidth}
    \begin{center}\includestandalone[width=.2\textwidth]{src/path}\end{center}
  \end{subfigure}
  \begin{subfigure}{0.5\textwidth}
    \includestandalone[width=.3\textwidth]{./src/tree}
  \end{subfigure}
\caption{Tree with first non explored node g X} 
\label{tree-ex1} 
\end{figure}

The step procedure aims to find the first non 

, it means essentially
that in the case of the And and Or terminal, we explore before the left 
and then the right branch.

\step is in charge of finding the first atom and reduce it.
In case of a call $c$, a new tree is built from the result of calling \backchain 
on $c$. \marginpar{dire che creiamo un bigor di bigand}
...
\subsubsection{The ! by graphics}

In a tree the scope of the cut operator is double.
