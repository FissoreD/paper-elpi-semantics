...
predicate passes the determinacy analysis, the predicate does not leave any
choice points.
...
In the tree we distinguish 6 main cases: Bot, Ok and Dead are special
meta-symbol representing respectively a failed, successful and dead state. While
the first two symbols are of immiediate understanding, we use Dead to represent
ghost state, that is, it allows to keep the structure of a tree from an
execution to another. A Dead tree is completely ignored by the intepretation of
the program. 

TA (acronym for tree-atom) is the constructor of atoms in the tree is a terminal
of the tree containg an atom.
...
The And non-terminal $A \land_{B_0} B$ represents a conjunction of two trees $A$
and $B$. We call $B_0$ the reset-point for $B$ and is used to resume the $B$
...
than binary), with right-binding priority. The Bot and Dead terminals are the
neutral element in the Or-list and OK is the neutral element of the And-list.
...
and \na that traverse the tree depth-first, left-to-right. Then, then \run
inductive makes the transitive closure of step \step and \na: it iterates the
calls to its auxiliary functions. In \cref{eq:run-types} we give the type
contrats of these symbols

\begin{align}\label{eq:run-types}
  &\text{\coqIn{Inductive step_tag := Expanded | CutBrothers| Failure | Success.}}\\
  &\text{\coqIn{Definition step : program -> sigma -> tree -> (step_tag * tree) := ...}}\\
  &\text{\coqIn{Definition next_alt : bool -> tree -> option tree := ...}}\\
  &\text{\coqIn{Inductive run (p : program): Sigma -> tree -> Sigma -> tree -> bool -> Type := ...}}
\end{align}
...
\begin{figure}
  ...
  \label{fig:pathend}
  \end{subfigure}
  \caption{isdead and pathend}
\end{figure}

A particular tree we want to identify is a is\_dead tree. This tree has the
property to never produce a solution and deals with terminal ending in Dead
states. Its definition is in \cref{fig:dead}.
In a non-dead tree, we get the first-to-beexplored node via path-end
the path\_end routine shown in \cref{fig:pathend}.
The path-end is either the tree itself if the tree is a terminal.
...
If the path-end $p$ of the lhs is a success then we look for 
the path-end in the rhs, otherwise we return $p$.
In \cref{tree-ex1} the path-end g X.
\begin{figure}
...
Below we define two special kind of trees depending on their path-end
...
\steptag together with the oupdated tree. The \steptag is a tag telling what
kind of internal tree step has been performed. It is either a call expansion
(\cI{Expanded}) or the evaluation of an internal cut (i.e. a cut appering below
a Or), a supeficial cut evaluation (\cI{CutBrothers}), i.e. a cut having only
And-nodes as fathers, \cI{Failure} or \cI{Success} if the tree is either
successT or failedT.
