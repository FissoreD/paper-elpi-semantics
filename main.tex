...
The call can either succeed (\textsc{CallE}) or fail (\textsc{FailE}). In the
latter case, a backchaining operation is mandatory: the first goal does not
succeed, we need to take the head of the alternatives, and make it the new list
of goals to be explored. In the former case, we need to build a new state and
this ask some attention. When we successfully call a predicate, multiple
rules may be used, by the SLD algorithm, the first rule should be
tried at first and then, if it fails that the other rules should be tried.
The translation of a rules to a list is done by taking its premises (the)
