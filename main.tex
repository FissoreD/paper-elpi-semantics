...
A particular tree we want to identify is a \isdead tree. This tree has the
property to never produce a solution: it is eiher the \deadC tree
or the lhs of \andC is dead or both branches of \orC are dead;
\isdead is defined in \cref{fig:dead}.
In a non-\isdead tree, we get the first-to-be-explored node via \pathend,
...
a \isdead tree, here, for example, we see how the interpreted is ignoring
a ghost state. 
...
a \orC), a supeficial cut evaluation (\cI{CutBrothers}), i.e. a cut having only
\andC-nodes as fathers, \cI{Failure} or \cI{Success} if the tree is either
\success or \failed. Stepping on a conjunction gives the 
