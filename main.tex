...
A rule (see~\cref{type:r}) is made a head of type term and a list of premises,
...
i.e. a tree that has been entirely.
...
auxiliary functions. In \cref{type:stepT,type:naT,type:runT} we give the types
...
\subsubsection{The \step procedure}
...
Free variables are those variables that appear in a tree; they are used in the
backchaining operation to refresh the variables in the program.  

The \steptag indicates the type of internal tree step that has been performed.
...
that is, it returnes the identity for \success and \failed tree.
...
Therefore, \step produces interesting results if the path-end 
of the input tree is either a call or a cut.

\textit{Call step} The interpretation of a call $c$ stars by calling the \marginpar{if we go right in the tree, the subst is the one in the or...}
...
has three main impacts: at first it is replaced by the \okC node, then
...
\begin{definition}[Soft-kill, \cutl]
  Given a successfull tree $t$, soft-kill replaces all the leaves of the tree with the node
  \koC except for the path in $t$ leading to the \okC node.
\end{definition}
...
routine interprets the cut since it is the node in its path-end. In the example
we have $4$ arrow tagged with the \cutl or \cutr symbols. The \cutl arrows go
left and soft-kill the pointed subtree, in particular, we can note that both
pointed subtree have a success node, this is beacuse, in order to evaluate the
cut in the figure, we need a successful path leading to it. The \cutl procedure
will keep the two \okC nodes since they are essential to reach the cut, and will
kill all the leaves in the other subtrees, for those specific subtrees, \cutl
behaves as \cutr. The \cutr procedure, instead, immediately starts by removing
all leaves in the trees pointed by the red arrows. \marginpar{dire che step non aggiunge mai nuovi dead}


\subsubsection{The \na procedure}

It is evident that the \step alone is not sufficient to reproduce entirely the
behavior of the full \elpiP solver. In particular, \step does not perform any
backtracking at all: it does not backtrack neither for failures, nor for
success, from \cref{lemma:success,lemma:failed}, \step returns the identity. To
do so, we have the \na procedure: its signature is provided in \cref{type:naT}
and its implementation in \cref{fig:na}.

The \na procedure takes a boolean and a tree and return a new tree if it
still contains an alternative. The intuition of \na is to introduce 
trasnform failed (or success) path into dead-path by inserting new Dead nodes.
 The boolean tells if there success leaves
should be 
...
this \koC node and make it a \deadC. This allows \step to continue the
...
\subsubsection{The \run inductive}
...
% \caption{Valid tree}
% \end{figure}
\subsection{\elpiP semantics}

TODO: dire che la semantica ad albero è puù faicle per le prove

The \elpiP interpreter is based on an operational semantics
