...
Once again, the most interesting cases to analyze are \orC and \andC.
...
formed by a disjunction of conjunctions.

For the \andC constructor, the lhs is required to be a valid tree. The shape
of the rhs depends on whether the lhs represents a success. If the lhs is not
...
the right-skewed tree containing the conjunctions of the atoms present in the
reset point $B_0$. In other words, the rhs coincides with the reset point.
If the lhs is a success tree, then the rhs must be a valid tree.
...
The idea of the elpi interpreter is to recieve a list of alternatives. The first
alternative is a list of goals. Four cases should be taken into account. They
are shown in \cref{fig:nur-rules}. In order to simplify the goal retrieval, we
have split the head of the alternatives for the tail to immediately find it in
the inductive. Note that an empty list of alternative represents, by definition,
a failing state. If the goal list is empty , then we have, by definition a success
