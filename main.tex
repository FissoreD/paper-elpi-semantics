...
Below we have an example of a simple elpi program which will be used in the
...
of these rules in the Rocq representation is straightforword.
...
\caption{Small elpi program example}
\label{fig:prog}
\end{figure}

\subsection{The cut operator}

The semantics of the cut operator adopted in the \textsc{Elpi} language
...
then we would have $(A_1, \ttl\ B); (A_2, B_0); \dots; (A_n, B_0)$.
...
Therefore, the two interesting cases of a tree the interpretation of 
trees with path-end equal to a call or a cut atom.

\textit{Call step} The interpretation of a call $c$ is performed by replacing
the call wrt the result of the \backchain $c$, then
if $l$ is empty then \koC tree is returned, otherwise
the call is replaced by right-skewed tree made of $n$ inner \orC nodes,
