...
alternative computations to explore and is well suited for studying
optimizations employed by standard \prologP abstract machines; moreover, it
closely models \elpiP's implementation. The second semantics is instead based
on an and-or tree, in which the branches pruned by the \emph{cut} operator are
made explicit.
...
produce, at run time, at most one result.
\end{abstract}
...
In our implementation, \run gives the garantee that the program interpretation
terminates: it always return a substitution. A diverging program will never
provide a solution. However, we can easily add a rule to treat this scenarion,
but studying diverging programs in our settings is not particularly worthy.
