...
\mvspace{}
\deftypes[eq:steptag]{tex_code/step_tag.tex}
\deftypes[eq:stepT]{tex_code/step_sig.tex}
\deftypes[eq:naT]{tex_code/next_alt.tex}
\deftypes[eq:runT]{tex_code/run_sig.tex}
...
\deflemma[lemma:success]{tex_code/failed_step.tex}
\mvspace{}
...
behavior of the full \elpiP solver. For example \step does not perform any
backtracking at all: it does not backtrack neither for failures, nor for
success. For example if a state is success then it would be interesting to find
the next solution. To do so, we have a \na procedure which aims to deadify
subtrees, that is it is allowed to transform \okC or \koC leaves into \deadC, so
