...
\begin{tikzpicture}[]
    \node (t1) at (0,0) {$t_0$};
    \node (t2) [right=2cm of t1]  {$t_1$};
    \node (e1) [below=2cm of t1]  {$a_0$};
    \node (e2) [right=2cm of e1]  {$a_1$};
    % letf to right arrows
    % \draw [->] (t1) -- (t2)node[pos=1,xshift=1pt,yshift=2pt]{*} node[midway, above]{\run'\ $t_0$};
    % \draw [->] (e1) -- (e2)node[pos=1,xshift=1pt,yshift=2pt]{*} node[midway, below]{\nur\ $a_0$};
    \draw [->] (t1) -- (t2) node[midway, above]{\run'\ $t_0$};
